...
def get_model(model_type, num_cls, input_dim, num_submodule=2):
    ...
    elif model_type == "mia_rnn":
        from models.attack_models import MIARnn
        model = MIATransformer(input_dim=num_cls, output_dim=1)
    elif model_type == "mia_me":
        from models.attack_models import MEMIA
        model = MEMIA(num_submodule=num_submodule, input_dim=num_cls, hidden_dim=1, output_dim=1)
    else:
        print(model_type)
        raise ValueError
    return model
...
def roc_plot(ROC_label, ROC_confidence_score, label='', plot=True):
    ...
    if plot == True:
        ...
        #print('auc=%.3f'%roc_auc + ', TPR=%.2f%%'%(low*100))
        #plt.plot(fpr, tpr, label=label+', auc=%.3f'%roc_auc + ', TPR=%.2f%%'%(low*100))
